...
utilities_monthly_water <- utilities %>% 
  ...
  mutate(expanded = if_else(is_water,
                            ...
                            list(tibble(utility = "Water",
                                        month = seq(month %m-% months(2), by = "1 month", 
                                                    length.out = 3),
                                        cost = cost / 3)),
...
```{r tithing}
sundays <- seq.Date(from = as.Date("2025-03-18"), to = today(), by = "day")
sundays <- sundays[weekdays(sundays) == "Sunday"]
tithing <- tibble(date = sundays, cost = 50) %>% 
  format_date()
```
...
total_costs <- full_join(total_cc, total_utils, by = "month") %>% 
  ...
  full_join(total_tithing, by = "month") %>% 
  drop_na() %>% 
  rename(cc = cost.x, utils = cost.y, ins = cost.x.x, groceries = cost.y.y, tithe = cost) %>% 
  mutate(rent = 2685, total = cc + utils + rent + ins + tithe) # Don't include groceries since they are already part of cc
total_costs_pivot <- total_costs %>% 
  pivot_longer(!month, names_to = "expense", values_to = "cost") %>% 
  mutate(expense = factor(expense, levels = c("cc", "groceries", "ins", "rent", "tithe", "utils",
                                              "total"),
                          labels = c("Credit Cards", "Groceries", "Insurance", "Rent",  "Tithing",
                                     "Utilities", "Total")))
