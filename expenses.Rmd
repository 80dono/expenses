...
From 01/09/2025 (when we started continuously tracking trips to the grocery store) to `r format(max(grocery$date), "%m/%d/%Y")`, we went to the grocery store `r sum(grocery$date >= as.Date("2025-01-09"))` times: an average of `r round(sum(grocery$date >= as.Date("2025-01-09")) / as.numeric(difftime(max(grocery$date), as.Date("2025-01-09"), units = "weeks")), 2)` times per week. All data and calculations going forward will only use the last 365 days as reference.
...
We will now plot the ratio of number of items and cost. Only stores that have been visited three or more times will be plotted, though all stores are included in the overall average cost line (dashed).
...
cc_total_cost %>% 
  ...
  scale_x_date(date_labels = "%b %Y") +
  # scale_y_continuous(n.breaks = 10) +
  scale_color_manual(values = c("PNC" = palette4[2], 
                                "Sapphire" = palette4[3],
                                "United" = palette4[1],
                                "USAA" = palette4[4],
                                "Total" = "black")) +
...
Note: the current date is `r today()`, so the last quarter only contains `r (month(today()) %% 3) - 1` full month(s) of utility data.
...
# Add dotted line projection for rest of current quarter?
# Ex. If the last monthly statement occurred 1/3 of the way through the quarter, multiply by 3 to get projection for the full quarter
# Add dotted line (separate geom_line) from value of last full quarter to value of projection
utilities_qtr_cost %>% 
