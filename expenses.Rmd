...
# Utilities
We will now examine the costs associated with utilities - specifically, electricity, water, Internet, and cell phone. Water is billed on a quarterly schedule, while the rest are billed monthly.
...
utilities <- utilities_sheet %>% 
  ...
  mutate(date = as.Date(date),
         year = year(date),
         month = floor_date(date, unit = "month"))
# Auto-generate fixed data each month for cell phone and Internet
```
...
# Try looking at costs on a quarterly basis, so all are on the same scale??
