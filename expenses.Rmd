...
tryCatch(
  {
    # Initial steps:
    #    - Create .Renviron file (not included in Git repo) with GARGLE_KEY_PATH and MY_EMAIL
    #    - Set use_oob = FALSE for initial run
    # gs4_auth(email = Sys.getenv("MY_EMAIL"), 
    #          scopes = "https://www.googleapis.com/auth/spreadsheets.readonly",
    #          cache = ".secrets")
    
    # Use Google Cloud Console service account
    gs4_auth(path = Sys.getenv("GARGLE_KEY_PATH"), cache = FALSE)
...
while (temp_month <= floor_date(today(), unit = "month")) {
  water_monthly_proj <- bind_rows(water_monthly_proj, 
                                  data.frame(utility = "Water", year = year(temp_month), 
                                             month = temp_month, real = FALSE, 
                                             cost = round(mean(filter(utilities_monthly_cost, utility == "Water")$cost), 2)))
...
if (max(filter(proj_data, utility == "Water")$quarter) < yearquarter(today())) {
  proj_data <- bind_rows(proj_data, 
                         data.frame(utility = "Water", quarter = yearquarter(today()), 
                                    cost = 0,
...
total_costs <- full_join(total_cc, total_utils, by = "month") %>% 
  ...
  mutate(rent = 2685, total = cc + utils + rent + ins) # Don't include groceries since they are already part of cc
