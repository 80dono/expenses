...
source("tools/generate_dynamic_plots.R")
...
# Total will be recalculated in the JavaScript section

total_cost_plot <- dynamic_total_plot(total_costs, 
                                      trace_order = c("cc", "groceries", "rent", "utils"), 
                                      trace_labels = c("Credit Cards", "Groceries", "Rent", "Utilities"),
                                      colors = palette4[1:4],
                                      time_unit = "month")

# Update final layout
total_cost_plot <- layout(total_cost_plot,
  title = "Overall Expenses by Month",
  xaxis = list(title = NULL, tickformat = "%b %Y"),
  yaxis = list(title = "Cost ($)"),
  legend = list(x = 0.5, xanchor = "center", y = -0.1, orientation = "h")
)
total_cost_plot
