...
# Utilities
We will now examine the costs associated with utilities - specifically, electricity, water, Internet, and cell phone. Water is billed on a quarterly schedule, while the rest are billed monthly. Internet is a fixed cost each month, while the others vary.
...
utilities_total_cost %>% 
  ...
  scale_x_date(date_labels = "%b %Y") +
  # scale_y_continuous(n.breaks = 10) +
  scale_color_manual(values = c("Electricity" = utilities_palette[2], 
                                "Water" = utilities_palette[3],
                                "Phone" = utilities_palette[1],
                                "Internet" = utilities_palette[4],
                                "Total" = "black")) +
  theme(legend.position = "bottom") +
