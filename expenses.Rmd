...
Below is the total expenditure per week for the last year, with each color in the stacked bar chart corresponding to the amount spent at a given store that week. Stores that have been visited less than five times are grouped as "Other".
...
**ADD AUTO INSURANCE AND TITHING EXPENSES**
...
insurance <- insurance_sheet %>% 
  ...
  arrange(date) %>% 
  filter(insurance %in% c("Medical", "Dental", "Vision"))

# Insurance info is incomplete! Missing auto, renters, etc.
...
total_costs <- full_join(total_cc, total_utils, by = "month") %>% 
  ...
  mutate(rent = if_else(month < as.Date("2026-03-01"), 2685, 4000), total = cc + utils + rent + ins) # Don't include groceries since they are already part of cc
