...
grocery_1year <- filter(grocery, date >= floor_date(today(), "month") - months(12))
```
...
Below is the total expenditure per week for the last year, with each color in the stacked bar chart corresponding to the amount spent at a given store that week. Stores that have been visited less than 5% of the time are grouped as "Other".

```{r grocery-week}
mean_weekly_cost <- mean(summarize(group_by(grocery_1year, week), 
                                   cost = sum(cost),
                                   .groups = "drop")$cost)
grocery_1year %>% 
  ...
  annotate("label", x = as.Date(min(grocery_1year$week) - days(14)), 
           y = mean_weekly_cost, 
...
The R-squared value for the third-order polynomial fit in the plot above is `r round(r2_week, 2)`.

#### Grocery Expenses by Month
We will now repeat each plot, this time aggregating over a month instead of week. The bar chart only includes data for the past year, but all other plots include data going back to January 2025.

```{r grocery-month}
mean_monthly_cost <- mean(summarize(group_by(grocery_1year, month), 
                                    cost = sum(cost),
                                    .groups = "drop")$cost)
grocery_1year %>% 
  group_by(year, month) %>% 
  ...
  geom_hline(yintercept = mean_monthly_cost,
             linetype = "dashed", linewidth = 1.5) +
  annotate("label", x = min(grocery_1year$month) - days(14), y = mean_monthly_cost, 
...
```{r grocery-cpi-data, include=FALSE, warning=FALSE}
# Get CPI data from the Bureau of Labor Statistics public API

# Series: Food at home in Middle Atlantic, all urban consumers, not seasonally adjusted
series_id <- "CUUR0120SAF11"

response <- POST(paste0("https://api.bls.gov/publicAPI/v1/timeseries/data/", series_id),
                 add_headers("Content-Type" = "application/json"))
content <- content(response, as = "parsed", type = "application/json")
cpi_groceries <- content$Results$series[[1]]$data %>% 
  map_dfr(as_tibble) %>% 
  filter(periodName %in% month.name, year >= 2025) %>% # Remove annual aggregates and unnecessary years
  mutate(year_month = yearmonth(paste(year, periodName)),
         cpi = as.numeric(value)) %>% 
  filter(!is.na(cpi)) %>% # Remove months with missing data (government shutdown, etc.)
...
grocery_month_cost <- grocery_month_summary %>% 
  ...
  geom_line(aes(text = paste0("<b>", format(month, "%B %Y"), "</b><br>Expenditure: ", 
                ...
                group = 1),
            col = "blue") +
  geom_line(data = cpi_groceries, aes(x = as.Date(year_month), 
                                      y = (cpi / 100) * median(grocery_month_summary$cost),
                                      text = paste0("<b>", format(year_month, "%B %Y"), 
                                                    "</b><br>CPI (scaled): ", 
                                                    dollar((cpi / 100) * median(grocery_month_summary$cost))), 
                                      group = 1), 
            col = "orange") +
  geom_smooth(method = "lm", formula = "y ~ x", se = FALSE,
              col = "black", linetype = "dashed") +
...
The orange line is the Consumer Price Index (CPI) times the median monthly grocery expenditure (to put it on a similar scale). The specific CPI series used is "Food at home in Middle Atlantic, all urban consumers, not seasonally adjusted", obtained from the Bureau of Labor Statistics' public API.
...
The R-squared value for the third-order polynomial fit in the plot above is `r round(r2_month, 2)`.

### Grocery Expenses by Store
The table below is sorted in descending order by number of trips.

```{r grocery-stores, out.width="50%", fig.align="default"}
grocery %>% 
  group_by(store) %>% 
  summarize(trips = n(), 
...
We will now plot the ratio of number of items and cost. Only stores that make up at least 5% of the trips will be plotted, though all stores are included in the overall average cost line (dashed). The plot is slightly jittered ($\pm$ 0.5 items) to improve visualization.

```{r item-cost-scatterplot, warning=FALSE, out.width="100%"}
grocery_item_cost <- grocery %>% 
  filter(store_label != "Other") %>% # Only include stores that have been visited 5+ times
  ggplot(aes(x = items, y = cost, col = store)) +
  geom_jitter(aes(group = 1,
                  text = paste0("<b>", store, "</b><br><i>", date,
                                "</i><br>Items: ", items, "<br>Expenditure: ", dollar(cost))),
              width = 0.5, height = 0, size = 0.75, alpha = 0.5) +
...
ggplotly(grocery_item_cost, tooltip = "text")
```
...
```{r shopper-analysis, warning=FALSE, out.width="100%"}
shoppers_plot <- grocery %>% 
  drop_na() %>% 
  ...
  geom_jitter(aes(group = 1, 
                  text = paste0("<b>", shopper, "</b><br><i>", date, "</i><br>Store: ", store, 
                                "<br>Items: ", items, "<br>Expenditure: ", dollar(cost))),
              width = 0.5, height = 0, size = 0.75, alpha = 0.5) +
  geom_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
  ...
  theme(legend.position = "inside", legend.position.inside = c(0.1, 0.9), 
        plot.caption = element_text(hjust = 0))
ggplotly(shoppers_plot, tooltip = "text")
```
...
The United card was cancelled in December 2025.
...
# Set credit card palette
cc_palette <- hue_pal()(length(levels(cc_total_cost$card)) - 1)
...
cc_cost_plot <- cc_total_cost %>% 
  ...
  geom_line() +
  scale_x_date(date_labels = "%b %Y") +
  scale_color_manual(values = c("Discover" = cc_palette[1], 
                                "Sapphire" = cc_palette[2],
                                "United" = cc_palette[3],
                                "USAA" = cc_palette[4],
                                "Total" = "black")) +
...
utilities <- utilities_sheet %>% 
  format_date(add_quarter = TRUE) %>% 
  arrange(date)
```

### Utility Expenses by Month
The water bill, which is quarterly as opposed to monthly, has been divided evenly among the three months it represents. This provides a better picture of the periodic fluctuations in utility costs throughout the year. Note, however, that this renders the Total incomplete for all months since the last water data point.
```{r total-utilities, warning=FALSE}
...
# Set utilities palette
utils_palette <- hue_pal()(length(levels(utilities_total_cost$utility)) - 1)
```

```{r monthly-utilities, warning=FALSE}
utilities_monthly_water <- utilities %>% 
  mutate(is_water = utility == "Water") %>%
  rowwise() %>% 
  mutate(expanded = if_else(is_water,
                            # Convert water from quarterly to monthly using average cost
                            list(tibble(utility = "Water",
                                        month = seq(month %m-% months(2), by = "1 month", length.out = 3),
                                        cost = cost / 3)),
                            list(tibble(utility = utility, month = month, cost = cost)))) %>%
  # Deconstruct back to a data frame
  ungroup() %>% 
  select(expanded) %>% 
  unnest(expanded) %>% 
  mutate(year = year(month)) %>% 
  group_by(utility, year, month) %>% 
  summarize(cost = sum(cost), .groups = "drop")

utilities_monthly_cost <- utilities_monthly_water %>% 
  group_by(year, month) %>% 
  summarize(cost = sum(cost), utility = "Total",
            .groups = "drop") %>% 
  bind_rows(utilities_monthly_water)
utilities_monthly_cost$utility <- factor(utilities_monthly_cost$utility, 
                                         levels = c(setdiff(levels(as.factor(utilities_monthly_cost$utility)),
                                                            "Total"), 
                                                    "Total"))

temp_month <- max(filter(utilities_monthly_cost, utility == "Water")$month) + months(1)
water_monthly_proj <- filter(utilities_monthly_cost, utility == "Water", month == temp_month - months(1))
while (temp_month <= floor_date(today(), unit = "month")) {
  water_monthly_proj <- bind_rows(water_monthly_proj, 
                                  tibble(utility = "Water", year = year(temp_month), 
                                         month = temp_month, real = FALSE, 
                                         cost = round(mean(filter(utilities_monthly_cost,
                                                                  utility == "Water")$cost), 2)))
  temp_month <- temp_month + months(1)
}

# Is there a way to make it so that when you remove water from the Plotly, the total line changes?

utilities_monthly_cost_plot <- utilities_monthly_cost %>% 
  ggplot(aes(x = month, y = cost, col = utility, group = utility,
             text = paste0("<b>", utility, "</b><br><i>", format(month, "%B %Y"), 
                           "</i><br>Cost: ", dollar(cost)))) +
  geom_vline(xintercept = as.Date("2026-02-01"), linetype = "dashed") +
  geom_line() +
  geom_line(data = water_monthly_proj, 
            aes(x = month, y = cost, col = utility, group = utility,
                text = paste0(paste0("<b>", utility, "</b><br><i>", 
                                     format(month, "%B %Y"), 
                                     "</i><br>Projected Cost: ", dollar(cost)))),
            linetype = 2, show.legend = FALSE) +
  # Re-add most recent (complete) month to supersede projection hover text
  geom_line(data = filter(utilities_monthly_cost, month == min(water_monthly_proj$month)),
            aes(text = paste0("<b>", utility, "</b><br><i>", format(month, "%B %Y"), 
                              "</i><br>Cost: ", dollar(cost)), 
                group = utility)) +
  scale_x_date(date_labels = "%b %Y") +
  scale_color_manual(values = c("Electricity" = utils_palette[2], 
                                "Gas" = utils_palette[5],
                                "Internet" = utils_palette[4],
                                "Phone" = utils_palette[1],
                                "Water" = utils_palette[3],
                                "Total" = "black")) +
  labs(title = "Utility Costs by Month",
       x = NULL, 
       y = "Cost ($)",
       col = NULL)
ggplotly(utilities_monthly_cost_plot, tooltip = "text") %>% 
  layout(legend = list(x = 0.5, xanchor = "center", y = -0.1,
                       orientation = "h"))
```

Note: there was no electricity bill for February 2026 as it did not fall within the 32-34-day billing cycle; electricity usage for that month is reflected in the March 2026 bill (on 3/3/2026).

### Utility Expenses by Quarter
Alternatively, instead of artificially converting water to a monthly expense, we can aggregate the other utilities to match the quarterly scale. We lose detail (particularly for electricity) in this approach but gain a better understanding of the true overall trend for all utilities with this standard basis of comparison.
...
proj_data <- utilities_qtr %>%
  filter((utility != "Water" & num_bills < 3 & quarter > yearquarter(today()) - 1)) %>%
  mutate(proj_cost = round(cost * (3 / num_bills), 2))
if (max(filter(proj_data, utility == "Water")$quarter) < yearquarter(today())) {
  proj_data <- bind_rows(proj_data, 
                         tibble(utility = "Water", quarter = yearquarter(today()),
                                cost = 0,
                                proj_cost = round(mean(filter(utilities_qtr_cost,
                                                              utility == "Water")$cost), 2)))
}

if (nrow(proj_data) > 1) {
  total_row <- tibble(utility = "Total",
  ...
  utilities_qtr_cost_plot <- utilities_qtr_cost %>% 
    ...
    geom_line(data = filter(utilities_qtr_cost, quarter == max(utilities_qtr_proj$quarter)),
...
Putting it all together, we will now plot all our major sources of expense together. Grocery totals will only include completed months, while credit cards and utilities may be incomplete for the current month. Note that while groceries are plotted separately, they are a part of the credit card expenses, and thus the total is not a sum of all the lines (doing so would double-count groceries). We only have insurance data going back to May 2025, so that is where this plot will start.

**ADD TITHING EXPENSES**
```{r insurance-data, include=FALSE}
insurance_sheet <- get_insurance_data(authenticated)
insurance <- insurance_sheet %>% 
  format_date(add_quarter = FALSE) %>% 
  arrange(date)

insurance_total_cost <- insurance %>% 
  group_by(year, month) %>% 
  summarize(cost = sum(cost), insurance = "Total",
            .groups = "drop") %>% 
  bind_rows(insurance)
```
...
total_utils <- filter(utilities_total_cost, utility == "Total") %>% 
  select(month, cost)
total_insurance <- filter(insurance_total_cost, insurance == "Total") %>% 
  select(month, cost)
total_costs <- full_join(total_cc, total_utils, by = "month") %>% 
  full_join(total_insurance, by = "month") %>% 
  full_join(select(grocery_month_summary, month, cost), by = "month") %>% 
  # Exclude the current month if incomplete
  drop_na() %>% 
  rename(cc = cost.x, utils = cost.y, ins = cost.x.x, groceries = cost.y.y) %>% 
  mutate(rent = if_else(month < as.Date("2026-03-01"), 2685, 3785.65), 
         hoa = if_else(month > as.Date("2026-02-01"), 146.22, NA_real_),
         total = rowSums(across(c(cc, utils, rent, hoa, ins)), na.rm = TRUE)) # Don't include groceries since they are already part of cc
total_costs_pivot <- total_costs %>% 
  pivot_longer(!month, names_to = "expense", values_to = "cost") %>% 
  mutate(expense = factor(expense, 
                          levels = c("cc", "groceries", "ins", "rent", "utils", "hoa", "total"),
                          labels = c("Credit Cards", "Groceries", "Insurance", "Rent", "Utilities",
                                     "HOA", "Total")))

# Set expenses palette
expenses_palette <- hue_pal()(length(levels(total_costs_pivot$expense)) - 1)

total_cost_plot <-  total_costs_pivot %>% 
  ggplot(aes(x = month, y = cost, col = expense, group = expense,
             text = paste0("<b>", expense, "</b><br><i>", format(month, "%B %Y"), 
                           "</i><br>Cost: ", dollar(cost)))) +
  geom_line() +
  scale_x_date(date_labels = "%b %Y") +
  scale_color_manual(values = c("Credit Cards" = expenses_palette[1], 
                                "Groceries" = expenses_palette[2],
                                "Insurance" = expenses_palette[3],
                                "Rent" = expenses_palette[4],
                                "Utilities" = expenses_palette[5],
                                "HOA" = expenses_palette[6],
                                "Total" = "black")) +
...
# Add expense line to legend (INCOMPLETE)
# color_map <- c(income_palette[1], income_palette[2], income_palette[3],
#                income_palette[4], income_palette[5], income_palette[6],
#                income_palette[7], "black", "red")
# names(color_map) <- c(levels(income_total$source), "Expenses")
color_map <- c(income_palette[1], income_palette[2], income_palette[3],
               income_palette[4], income_palette[5], income_palette[6],
               income_palette[7], "black")
...
income_plot_month <- income_month %>% 
  ...
  geom_line() +
  geom_point(size = 0.5) +
  # geom_line(data = mutate(total_costs, total = total - ins) %>% select(month, total),
  #           aes(x = month, y = total, group = 1,
  #               text = paste0("<b>Expenses</b><br><i>", format(month, "%B %Y"), 
  #                             "</i><br>Amount: ", dollar(total))),
