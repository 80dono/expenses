...
total_cost_plot <-  total_costs_pivot %>% 
  ...
  scale_color_manual(values = c("Credit Cards" = expenses_palette[1], 
                                ...
                                "Rent" = expenses_palette[3],
                                "Utilities" = expenses_palette[4],
                                "Total" = "black")) +
...
# Income
There are various sources of income, ranging from paychecks to gifts to reimbursements (e.g. work travel, taxes). Complete data ranges back to approximately May 2025.
...
We will display income by month to better contextualize what is encompassed by the total. Due to biweekly (rather than monthly) paychecks, the total will fluctuate intermittently.

```{r monthly-income, out.width="100%"}
income_month <- income %>% 
  group_by(year, month, source) %>% 
  summarize(amount = sum(amount), inputs = n(), .groups = "drop") %>% 
  bind_rows(filter(income_total, source == "Total"))

income_sources <- levels(income_total$source)
