...
grocery_month_cost <- grocery_month_summary %>% 
  ...
  geom_line(data = cpi_groceries, aes(x = as.Date(year_month), 
                                      y = (cpi / 100) * mean(grocery_month_summary$cost)), 
                                      col = "orange") +
  geom_smooth(method = "lm", formula = "y ~ x", se = FALSE,
              col = "black", linetype = "dashed") +
  scale_x_date(date_labels = "%b %Y") +
...
The orange line is the Consumer Price Index (CPI) times the mean monthly grocery expenditure (to put it on a similar scale). The specific CPI series used is "Food at home in Middle Atlantic, all urban consumers, not seasonally adjusted", obtained from the Bureau of Labor Statistics' public API.
...
```{r shopper-analysis, warning=FALSE}
...
utilities_monthly_cost <- utilities_monthly_water %>% 
  ...
  # Reset utility order so Total is at the end
...
while (month(temp_month) <= month(today())) {
