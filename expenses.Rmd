...
total_costs <- full_join(total_cc, total_utils, by = "month") %>% 
  ...
  # ADD HOA EXPENSE - $146.22
  mutate(rent = if_else(month < as.Date("2026-03-01"), 2685, 3785.65), total = cc + utils + rent + ins) # Don't include groceries since they are already part of cc
total_costs_pivot <- total_costs %>% 
  pivot_longer(!month, names_to = "expense", values_to = "cost") %>% 
  mutate(expense = factor(expense, levels = c("cc", "groceries", "ins", "rent", "utils", "total"),
