...
library(scales, include.only = "dollar")
library(plotly)
library(DT)
library(scales)
library(tsibble)
...
Below are plots of the number of trips per week and expenditure per week over time, to see if there is any directional or seasonal change. Only includes full weeks (does not include the current week, unlike the plot above).

```{r grocery-week-trips-cost, out.width="50%", fig.align="default", warning=FALSE}
...
grocery_week_trips <- grocery_week_summary %>% 
  ...
  labs(title = "Trips to the Grocery Store by Week",
       x = "Week", 
       y = "Number of Trips")
ggplotly(grocery_week_trips, tooltip = "text")
...
grocery_week_cost <- grocery_week_summary %>% 
  ...
  labs(title = "Grocery Expenses by Week",
       x = "Week", 
       y = "Expenditure ($)")
ggplotly(grocery_week_cost, tooltip = "text")
```
...
ggplotly(grocery_week_trips_cost, tooltip = "text")
```
...
```{r grocery-month-trips-cost, out.width="50%", fig.align="default", warning=FALSE}
...
grocery_month_trips <- grocery_month_summary %>% 
  ...
  labs(title = "Trips to the Grocery Store by Month",
       ...
       y = "Number of Trips",
       col = "Expenditure ($)")
ggplotly(grocery_month_trips, tooltip = "text")
...
grocery_month_cost <- grocery_month_summary %>% 
  ...
  labs(title = "Grocery Expenses by Month",
       x = "Month", 
       y = "Expenditure ($)")
ggplotly(grocery_month_cost, tooltip = "text")
```
...
grocery_month_trips_cost <- grocery_month_summary %>% 
  ...
  labs(title = "Monthly Cost by Number of Trips",
       ...
       x = "Trips",
       y = "Monthly Total ($)")
ggplotly(grocery_month_trips_cost, tooltip = "text")
```
...
ggplotly(cc_cost_plot, tooltip = "text") %>% 
  layout(legend = list(x = 0.4, xanchor = "center", y = -0.1,
                       orientation = "h"))
```
...
utilities_qtr_cost_plot <- utilities_qtr_cost %>% 
  ...
  labs(title = "Utility Costs by Quarter",
       subtitle = paste0("Projections based on ", 
                         filter(utilities_qtr_proj, utility == "Electricity", 
                                quarter == max(quarter))$num_bills, 
                         " months of bills from the current quarter."),
       x = NULL, 
       y = "Cost ($)",
       col = NULL)
