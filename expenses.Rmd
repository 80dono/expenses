...
Note: the current date is `r today()`, so the last quarter only contains `r (month(today()) %% 3) - 1` full month(s) of utility data. As such, we have scaled the data to the full quarter for each utility (except water, which uses the average of the previous water bills) and taken the sum of those scaled projections to get the total.
...
utilities_qtr_cost <- utilities_qtr %>% 
  group_by(quarter) %>% 
  summarize(cost = sum(cost), num_bills = n(), utility = "Total",
            .groups = "drop") %>% 
...
utilities_qtr_proj <- utilities_qtr %>% 
  ...
  bind_rows(data.frame(utility = "Total", 
            ...
            mutate(filter(utilities_qtr_cost, quarter == .$quarter[1] - 1),
                   proj_cost = cost)) # Note: num_bills for Total will be incorrect
...
utilities_qtr_cost %>% 
  ...
  labs(title = "Utility Costs by Quarter",
       subtitle = paste0("Projections based on ", filter(utilities_qtr_proj, utility == "Electricity", quarter == max(quarter))$num_bills, " months of bills from the final quarter."),
       x = "Quarter", 
       y = "Cost ($)",
       col = "Utility")
```
