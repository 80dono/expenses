...
grocery <- grocery_sheet %>% 
  ...
  mutate(store = as.factor(store),
         ...
         total = 0)
grocery$total[1] <- grocery$cost[1]
for (i in c(2:nrow(grocery))) {
  grocery$total[i] <- grocery$cost[i] + grocery$total[i - 1]
}
...
```{r grocery-stores-month}
grocery2025 %>% 
  mutate(month = month(date, label = TRUE)) %>% 
  group_by(month) %>% 
  mutate(monthly_total = sum(cost)) %>% 
